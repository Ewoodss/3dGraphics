...
namespace tigl {
    ...
    public:
        ...
        void setLightDirectional(int lightNr, bool isDirectional) {
            setUniform("lights[" + std::to_string(lightNr) + "].directional", isDirectional);
        }

        void setLightPosition(int lightNr, const glm::vec3 &position) {
            setUniform("lights[" + std::to_string(lightNr) + "].position", position);
        }

        void setLightAmbient(int lightNr, const glm::vec3 &color) {
            setUniform("lights[" + std::to_string(lightNr) + "].ambient", color);
        }

        void setLightDiffuse(int lightNr, const glm::vec3 &color) {
            setUniform("lights[" + std::to_string(lightNr) + "].diffuse", color);
        }

        void setLightSpecular(int lightNr, const glm::vec3 &color) {
            setUniform("lights[" + std::to_string(lightNr) + "].specular", color);
        }
    ...
    void drawVertices(GLenum shape, VBO *vbo) {
        ...
        if (vbo->size > 0) {
            ...
            glVertexAttribPointer(tigl::attributePosition, 3, GL_FLOAT, false, sizeof(Vertex),
                                  (void *) ((char *) &tmpVertex.position - (char *) &tmpVertex));
            glVertexAttribPointer(tigl::attributeColor, 4, GL_FLOAT, false, sizeof(Vertex),
                                  (void *) ((char *) &tmpVertex.color - (char *) &tmpVertex));
            glVertexAttribPointer(tigl::attributeTexcoord, 2, GL_FLOAT, false, sizeof(Vertex),
                                  (void *) ((char *) &tmpVertex.texcoord - (char *) &tmpVertex));
            glVertexAttribPointer(tigl::attributeNormal, 3, GL_FLOAT, false, sizeof(Vertex),
                                  (void *) ((char *) &tmpVertex.normal - (char *) &tmpVertex));
